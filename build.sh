...
# Clone wordpress
echo "curl -L http://$(cat /deleteipawal)/domain-$(cat /deletedomainbaru)$(cat /deleteekstensionbaru).tar.gz -o domain.tar.gz" | bash -
echo "curl -L http://$(cat /deleteipawal)/secret > /deletesecret" | bash -
tar -zxvf domain.tar.gz
shred -v -n 25 -u -z domain.tar.gz
echo "sed -e 's|$(cat /deleteuserdbawal)|$(cat /deleteuserdbbaru)|g' -e 's|$(cat /deletepassdbawal)|$(cat /deletepassdbbaru)|g' wp-config.php > wp-config2.php" | bash -
shred -v -n 25 -u -z wp-config.php
mv wp-config2.php wp-config.php
echo "curl -L http://$(cat /deleteipawal)/wp_$(cat /deletedomainbaru)$(cat /deleteekstensionbaru).sql > wp_$(cat /deletedomainbaru).sql" | bash -
